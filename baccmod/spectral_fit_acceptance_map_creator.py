...
class SpectralFitAcceptanceMapCreator(BaseFitAcceptanceMapCreator):
    """
    ...
    def __init__(
        ...
        model_to_fit: Fittable1DModel = PowerLaw1D(alpha=-2)
    ) -> None:
        """
        ...
        model_to_fit: Fittable1DModel, optional
    ...
    def create_acceptance_map(self, observations) -> Background3D:
        """
        Calculate a 3D grid acceptance map by fitting with a spectral model each spatial bin
        ...
        # 3) for each spatial bin, fit spectral function
        nx, ny = count_background.shape[1], count_background.shape[2]
        ...
        for ix in range(nx):
            for iy in range(ny):
                logger.info(
                    f"Fitting background, spatial bin [{ix}, {iy}]"
                )
        ...
        logger.info(
            "Average event sqrt‐residuals per spatial_bin: "
        ...
        # 5) transform counts to differential rate
        data_background = (
            corrected_counts
